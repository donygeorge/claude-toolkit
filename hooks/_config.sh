...
if [ -f "$CACHE_FILE" ]; then
  ...
  if [ -f "$TOML_FILE" ] && [ "$TOML_FILE" -nt "$CACHE_FILE" ]; then
    echo "Warning: toolkit.toml is newer than toolkit-cache.env. Run: python3 generate-config-cache.py --toml .claude/toolkit.toml --output .claude/toolkit-cache.env" >&2
  fi
fi
