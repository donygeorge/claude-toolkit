...
If `.claude/settings.json` already exists and `.claude/settings-project.json` does not, init automatically preserves your existing settings by copying them into `settings-project.json` (the project overlay). This ensures your custom permissions, hooks, deny rules, and MCP servers are retained after the toolkit's three-tier merge. Backups are created as `settings.json.pre-toolkit` and `.mcp.json.pre-toolkit`.
...
The project overlay is optional. If you run `init` on a project with an existing `settings.json`, the toolkit automatically creates `settings-project.json` from your existing settings. You can also create it manually.

The merge is deterministic (sorted keys, 2-space indent). Permission arrays are deduplicated. MCP server entries use replacement semantics (project version replaces base version entirely). The result is written to `.claude/settings.json` and `.mcp.json`.
